...
def init_args(args):
    ...
    args.visualPathAVA = os.path.join(args.DATA.dataPathAVA, 'frames')
...
def preprocess_AVA(args):
    ...
    # |   ├── test_entity.json (for designating target speaker in multispeaker setting)
    # |   ├── test_ts.json (context speaker information)
    ...
    # |   ├── train_entity.json (for designating target speaker in multispeaker setting)
    # |   ├── train_ts.json (context speaker information)
    # │   ├── trainval_file_list.txt (name of the train/val videos)
    # │   ├── val_loader.csv (The csv file we generated to load data for validation)
    # |   ├── val_entity.json (for designating target speaker in multispeaker setting)
    # |   ├── val_ts.json (context speaker information)
...
def download_csv():
    ...
    Link = "1h8DISV9sYHGi2CsDI7PXK4kXmS2_kjEg"
    cmd = "gdown --id %s -O %s" % (Link, 'talknce_csv.zip')
    subprocess.call(cmd, shell=True, stdout=None)
    cmd = "unzip %s -d %s" % ('talknce_csv.zip', './csv')
    subprocess.call(cmd, shell=True, stdout=None)
    os.remove('talknce_csv.zip')
