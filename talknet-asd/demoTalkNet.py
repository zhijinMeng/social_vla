...
parser.add_argument('--chunkDuration',         type=float, default=0, help='Process video in sequential chunks (seconds), 0 means disabled')
parser.add_argument('--chunkStride',           type=float, default=0, help='Chunk start stride in seconds, 0 means same as chunkDuration')
parser.add_argument('--chunkRuns',             type=int, default=0,   help='Number of chunks to run, 0 means auto until video end')
...
def inference_video(args, detector = None):
	# GPU: Face detection, output is the list contains the face location and score in this frame
	DET = detector if detector is not None else S3FD(device='cuda')
...
def evaluate_network(files, args, speakerModel = None):
	# GPU: active speaker detection by pretrained TalkNet
	if speakerModel is None:
		s = talkNet()
...
def get_video_duration(videoPath):
...
def run_pipeline_once(args, detector = None, speakerModel = None):
...
\n" %(args.pyworkPath))
	...
	faces = inference_video(args, detector = detector)
...
\n" %args.pycropPath)
	...
	files.sort()
	scores = evaluate_network(files, args, speakerModel = speakerModel)
...
def main():
	# Default: original single-run behavior.
	if args.chunkDuration <= 0 or args.evalCol == True:
		run_pipeline_once(args)
		return

	# Chunk mode: sequential windows, with one-time model/detector loading.
	chunkStride = args.chunkStride if args.chunkStride > 0 else args.chunkDuration
	videoDur = get_video_duration(args.videoPath)
	if videoDur <= 0:
		print("Cannot read video duration, fallback to single run.")
		run_pipeline_once(args)
		return

	if args.chunkRuns > 0:
		nRuns = args.chunkRuns
	else:
		remaining = max(videoDur - args.start, 0)
		nRuns = max(int(math.ceil(remaining / chunkStride)), 1)

	print("Chunk mode enabled: duration=%.3fs, stride=%.3fs, runs=%d"%(args.chunkDuration, chunkStride, nRuns))

	# Load once
	detector = S3FD(device='cuda')
	speakerModel = talkNet()
	speakerModel.loadParameters(args.pretrainModel)
	speakerModel.eval()
	print("Loaded S3FD + TalkNet once for chunk loop")

	baseSave = args.savePath
	baseStart = float(args.start)
	for idx in range(nRuns):
		args.start = baseStart + idx * chunkStride
		if args.start >= videoDur:
			break
		args.duration = args.chunkDuration
		args.savePath = os.path.join(baseSave, 'chunks', 'chunk_%04d'%idx)
		print("\n[Chunk %d] start=%.3fs, duration=%.3fs, save=%s"%(idx, args.start, args.duration, args.savePath))
		run_pipeline_once(args, detector = detector, speakerModel = speakerModel)
